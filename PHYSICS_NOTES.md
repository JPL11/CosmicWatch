...
- **Coincident events have higher ADC (p50 334 vs 183):** requiring *both* internal panels to fire
  selects genuine through-going particles (real tracks, more light) and rejects single-panel noise.
  **Energy deposition is the physical discriminator** — which is exactly why an ADC threshold already
  captures most of the signal and a fancier model cannot beat it. The ML "ceiling" is physics, not a
  modeling failure.
...
- **Inter-arrival CV = 0.75 (sub-Poissonian):** cosmic-ray arrivals are intrinsically a **Poisson
  (memoryless, random)** process. We observe *less* spread than Poisson because the detector's
  **dead-time** after each hit regularizes the intervals. The deviation is a real instrument-physics
  signature, not noise.
...
- A **single sensor / single node** can do rate, energy-spectrum, and Poisson/dead-time physics. It
